...
export default function SwitchNetworkButton() {
  ...
  return (
    <div className="relative">
      <button
        className="relative flex min-h-[45px] min-w-[12px] w-full md:w-auto items-center justify-center rounded-full border border-[#1DDAFF] bg-transparent py-2.5 md:py-3 px-3 md:px-4 text-xs md:text-sm font-semibold uppercase leading-none text-white transition-all duration-300 ease-in-out hover:border-[#fff3] hover:bg-[#f264ff33] disabled:pointer-events-none disabled:opacity-40"
        type="button"
        ...
        <div className="flex items-center justify-between whitespace-nowrap">
          {config.chainDetails[chain.id] && (
            <img
              ...
              alt={`${chain.id}`}
              className="h-[18px] md:h-[22px]"
            />
          )}
          <span className="mx-1.5 md:mx-2 text-xs md:text-sm">Buy with {chain?.nativeCurrency.symbol}</span>
          <img src="/img/angle-down.svg" alt="angle-down" className="h-3 w-3 md:h-4 md:w-4" />
        </div>
      </button>
      {dropdownOpen && (
        <ul
